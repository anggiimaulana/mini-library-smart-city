...
class SmartCityApp {
    ...
    async renderHomePage() {
        ...
        if (welcomeTitle) {
            welcomeTitle.textContent = `Welcome, ${this.currentUser.name}!`;
        }
        if (welcomeSubtitle) {
    ...
    async renderPillarPage() {
        ...
        try {
            ...
            // document.getElementById('pillarImage').style.backgroundImage = url('${pillarData.image}');
            document.getElementById('pillarDescription').innerHTML = pillarData.description;
            document.getElementById('pillarChallenges').innerHTML = pillarData.challenge_solution || 'No data';
            document.getElementById('pillarTechnology').innerHTML = pillarData.technology || 'No data';
            document.getElementById('pillarImplementation').innerHTML = pillarData.implementation || 'No data';
            ...
            this.showToast('Failed to load pillar data', 'error');
    ...
    async renderReferencesPage() {
        try {
            ...
            this.showToast('Failed to load references', 'error');
    ...
    async loadProgressData() {
        try {
            ...
            if (progressText) {
                progressText.textContent = `${progressData.completed_count} out ${progressData.total_references} references have been read`;
            }
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex items-center justify-between">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-primary-100 text-primary-800">
                                    ...
                                    ${pillar.resources_count || 0} Reference
                                </span>
            ...
            if (statsSection) {
                statsSection.innerHTML = `
                    <div class="bg-white rounded-2xl shadow-lg p-6 text-center border-l-4 border-primary-500">
                        ...
                        <div class="text-sm text-gray-600 uppercase tracking-wide">Smart City Pillar</div>
                    </div>
    ...
    async loadStudyPrograms(majorId) {
        ...
        if (!majorId) {
            studyProgramSelect.innerHTML = '<option value="">Choose a Study Program</option>';
        ...
        try {
            ...
            this.showToast('Failed to load study programs', 'error');
        }
    }
    ...
    async handleRegister(data) {
        ...
        try {
            ...
            if (response.success) {
                ...
                this.showToast('Registration successful! Save your unique code.', 'success');
            }
        } catch (error) {
            if (error.errors) {
    ...
    async handleLogin(data) {
        ...
        try {
            ...
            if (response.success) {
                ...
                this.showToast(`Welcome, ${response.user.name}!`, 'success');
    ...
    viewReference(url) {
        if (!url || url === '#') {
            this.showToast('Reference link is not available', 'warning');
            return;
        }
        ...
        try {
            ...
            if (response.success) {
                if (isCurrentlyRead) {
                    ...
                    button.innerHTML = '<i class="fas fa-bookmark mr-1"></i>Mark as read';
                } else {
                    ...
                    button.innerHTML = '<i class="fas fa-check mr-1"></i>Read';
                }
                ...
                this.showToast(
                    isCurrentlyRead ? 'Reference marked as unread' : 'Reference marked as read',
                    'success'
                );
            }
        } catch (error) {
            this.showToast('Failed to change reference status', 'error');
        ...
        try {
            ...
            if (progressData.percentage === 100) {
                ...
                if (certificateSection && certificateSection.classList.contains('hidden')) {
                    certificateSection.classList.remove('hidden');
                    this.showToast('🎉 Congratulations! You have completed all the learning!', 'success');
                }
            }
        } catch (error) {
    ...
    updateReferencesStats(references) {
        ...
        if (journalCount) {
            journalCount.textContent = references.filter(r => 
                r.source_category.name === 'Journal'
            ).length;
        }
        if (bookCount) {
            bookCount.textContent = references.filter(r => 
                r.source_category.name === 'Book'
            ).length;
        }
    }
   ...
   populateFilters(references) {
        ...
        if (pillarFilter) {
            ...
            pillarFilter.innerHTML = '<option value="">All Pillar</option>' +
                pillars.map(pillar => 
            ...
            <tr data-pillar="${resource.content.title}" 
                ...
                <td class="px-6 py-4">
                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                        ${resource.source_category.name === 'Journal' ? 'bg-blue-100 text-blue-800' : 
                          resource.source_category.name === 'Book' ? 'bg-green-100 text-green-800' : 
                ...
                <td class="px-6 py-4">
                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                        ...
                        ${this.readReferences.has(resource.id.toString()) ? 'Read' : 'Unread'}
                    </span>
                </td>
                <td class="px-6 py-4">
            ...
            <div class="flex items-start justify-between p-6 bg-gray-50 hover:bg-white rounded-xl border hover:border-primary-200 transition-all ${isRead ? 'bg-green-50 border-l-4 border-green-400' : ''}" data-resource-id="${resource.id}">
                ...
                <div class="flex space-x-2 ml-4">
                    <button onclick="app.viewReference('${resource.link}')" 
                        ...
                        <i class="fas fa-external-link-alt mr-1"></i>See
                    </button>
                    <button onclick="app.toggleReadStatus(${resource.id}, this)"
                        ...
                        <i class="fas ${isRead ? 'fa-check' : 'fa-bookmark'} mr-1"></i>${isRead ? 'Read' : 'Mark as read'}
                    </button>
                </div>
            </div>
        `;
...
function downloadCertificate() {
    ...
    html2canvas(certificateElement, {
        ...
        link.download = `certificate-mini-library-smart-city-${app.currentUser.nim}.png`;
        link.href = canvas.toDataURL();
        link.click();
    }).catch(error => {
        console.error('Error generating certificate image:', error);
        app.showToast('Failed to download the certificate', 'error');
    });
}
...
function generatePDF() {
    ...
    html2canvas(certificateElement, {
        ...
        app.showToast('Failed to download PDF', 'error');
    });
}
...
window.addEventListener('error', function(event) {
    ...
    if (window.app) {
        app.showToast('An application error has occurred', 'error');
    }
});
...
window.addEventListener('unhandledrejection', function(event) {
    ...
    if (window.app) {
        app.showToast('A network error has occurred', 'error');
    }
